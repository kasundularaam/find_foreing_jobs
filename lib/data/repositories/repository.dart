...
import '../models/ffj_user.dart';
import '../models/vacancy.dart';

class Repository {
  static Future<FFJUser> getSlbfeUser({required String nic}) async {
    try {
      ...
      if (res.statusCode == 200) {
        return FFJUser.fromJson(res.body);
      }
  ...
  static Future<List<String>> getConnectionIds({required String nic}) async {
    FFJUser user = await getSlbfeUser(nic: nic);
  ...
  static Future<FFJUser> getCitizenById({required String uid}) async {
    try {
      final res = await http.get(
        Uri.parse(
  ...
  static Future<FFJUser> register({required NewUser newUser}) async {
    try {
      final res = await http.post(
          Uri.parse(
  ...
  static Future<List<ConnectionUser>> getConnectUsers(
    ...
    try {
      ...
      for (var userId in userIds) {
        FFJUser user = await getCitizenById(uid: userId);
        connectionUsers.add(
          ConnectionUser(
            id: user.id,
            name: user.name,
